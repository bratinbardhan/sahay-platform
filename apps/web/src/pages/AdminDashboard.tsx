...
export function AdminDashboard({ user, token, onLogout }: AdminDashboardProps) {
  ...
  if (loading && !overview && !usersPage) {
    return (
      <div className="flex min-h-screen bg-slate-50 items-center justify-center">
        <p className="text-slate-800 text-lg">Loading admin console…</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 md:p-10 overflow-x-hidden animate-in">
      {/* Navigation bar */}
      ...
      <header className="mt-6 mb-6">
        <h1 className="text-3xl sm:text-4xl font-bold text-slate-800">Admin Console</h1>
        <p className="text-slate-800/80 text-base mt-1">
          Live platform telemetry, user management, and cognitive analytics.
        </p>
      </header>

      {error ? (
        <div className="mb-6 rounded-xl border-2 border-teal-600 bg-amber-50 px-4 py-3 text-sm text-red-700 flex items-center justify-between gap-3">
          <span>{error}</span>
          <button
            type="button"
            onClick={() => {
      ...
      {/* ─── Top Metrics Row ─────────────────────────────────────────── */}
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 animate-fade-up">
        <MetricCard
          label="Online Now"
      ...
      {safeOverview ? (
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 animate-fade-up">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm transition-shadow">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm transition-shadow lg:col-span-2">
            <div className="flex items-center gap-2 text-slate-800 mb-4">
              <RefreshCw size={22} />
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 shadow-sm animate-fade-up">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="p-2 rounded-lg border-2 border-slate-300 text-slate-800 hover:bg-slate-200 transition-colors"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {(usersPage?.items ?? []).map((row) => (
                <tr key={row.id} className="border-b border-slate-300/20 hover:bg-slate-50 transition-colors">
                  <td className="p-2 font-medium text-slate-800">{row.full_name}</td>
                  <td className="p-2 text-slate-800/80 break-all">{row.email}</td>
                  <td className="p-2 text-slate-800">{row.role}</td>
                  <td className="p-2">
                    <TierBadge tier={row.tier} />
                  </td>
                  <td className="p-2 text-slate-800/70">
              ...
              {usersPage && usersPage.items.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-6 text-center text-slate-800/60">
                    No users found.
                  </td>
                </tr>
              ) : null}
        ...
        {usersPage && usersPage.pages > 1 ? (
          <div className="flex items-center justify-between mt-4 text-sm text-slate-800">
            <span>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1 rounded-lg border-2 border-slate-300 disabled:opacity-40 hover:bg-slate-200 transition-colors"
              >
                Prev
              </button>
              <button
...
function TierToggle({
  ...
  return (
    <div className="inline-flex rounded-xl border-2 border-slate-300 overflow-hidden">
